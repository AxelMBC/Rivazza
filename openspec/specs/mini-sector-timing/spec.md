...
Because the lap-identity palette repeats across a long session, color alone SHALL NOT be the only way to identify a slice's owner: hovering a slice SHALL show a readout naming the slice, its owning lap, that lap's time for the slice, and its invalidity when applicable. The readout SHALL work with hover alone — no click, no keyboard, no window focus — and SHALL disappear when the pointer leaves the strip.

#### Scenario: Identifying an owner in a long session
- **WHEN** the session has more laps than the identity palette has colors and the driver hovers a slice
