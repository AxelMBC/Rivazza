    ...
    const draw = () => {
      ...
      for (const trace of TRACES) {
        ...
        for (const sample of history) {
          ...
          const y =
            PAD +
            (1 - Math.min(1, Math.max(0, sample[trace.key]))) *
              (height - PAD * 2);
          if (!started) {
            ctx.moveTo(x, y);
            started = true;
